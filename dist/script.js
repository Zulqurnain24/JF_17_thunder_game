...
function Hero () {
  ...
  heroMaterial = new THREE.MeshPhongMaterial({
    color,
    opacity: 0.0,
    transparent: true,
  });
  ...
  function createObject () {
    ...
    loader.load( 'jf_17.dae', function ( collada ) {

    var jf_17 = collada.scene;
    jf_17.castShadow = true;
    jf_17.rotation.x = 1.5708;
    jf_17.rotation.y = 2 * 1.5708;
    jf_17.scale.set( 2, 2, 2 );
    jf_17.translateZ( -2.5 );
    hero.add( jf_17 );
...
function PowerUp () {
  ...
  objectGeometry = new THREE.CylinderGeometry( 0.25, 2.5, 3, 3, 0, 10, 20);
...
function initGame () {
  ...
  skyMaterial = new THREE.MeshBasicMaterial( {
    map: THREE.ImageUtils.loadTexture( 'sun_set.jpg' ),
    depthWrite: false,
    side: THREE.BackSide
  } );
